...
def learnPredictor(trainExamples: List[Tuple[Any, int]], validationExamples: List[Tuple[Any, int]],
    ...
    for epoch in range(numEpochs):
        for x, y in trainExamples:
            ...
            scale = alpha * y/(1 + math.exp(y * score))
            increment(weights, scale, phi)

        # report train/validation error after each epoch
        def predictor(x):
            phi = featureExtractor(x)
            return 1 if dotProduct(phi, weights) >= 0 else -1

        trainError = evaluatePredictor(trainExamples, predictor)
        valError = evaluatePredictor(validationExamples, predictor)

        print(f"Training Error: ({epoch} epoch): {trainError}")
        print(f"Validation Error: ({epoch} epoch): {valError}")
